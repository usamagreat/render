...
module.exports = ({ queries, uuidv4, sendToDevice, broadcastToDashboards }) => {
    ...
    router.post('/device/:id/command', (req, res) => {
        try {
            ...
            const cmdId = uuidv4();
            queries.insertCommand(cmdId, req.params.id, type, payload ? JSON.stringify(payload) : null);
    ...
    router.get('/device/:id/commands/pending', (req, res) => {
        const device = queries.getDevice(req.params.id);
        if (!device) return res.status(404).json({ error: 'Device not found' });
        res.json(queries.getPendingCommands(req.params.id) || []);
    });

    // Mark command as done (HTTP fallback)
    router.patch('/command/:cmdId/done', (req, res) => {
        queries.updateCommandStatus('done', req.params.cmdId);
        res.json({ success: true });
    });

    // Get command history for a device
    router.get('/device/:id/commands', (req, res) => {
        res.json(queries.getCommandHistory(req.params.id) || []);
    });
