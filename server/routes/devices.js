...
module.exports = ({ queries, uuidv4 }) => {
    ...
    router.post('/device/register', (req, res) => {
        try {
            ...
            const id = uuidv4();
            const token = uuidv4();
            queries.insertDevice(id, name, token, model || '', brand || '', android_version || '');
    ...
    router.get('/devices', (req, res) => {
        res.json(queries.getAllDevices() || []);
    });
    ...
    router.patch('/device/:id/name', (req, res) => {
        const { name } = req.body;
        if (!name) return res.status(400).json({ error: 'name is required' });
        const success = queries.updateDeviceName(name, req.params.id);
    ...
    router.delete('/device/:id', (req, res) => {
        queries.deleteDevice(req.params.id);
        res.json({ success: true });
    });
