    ...
    ws.on('message', (rawMsg) => {
        try {
            ...
            switch (msg.type) {
                ...
                case 'stream': {
                    if (!deviceId) return;
                    // Pass real-time streams (camera_frame, audio_chunk) directly to dashboards
                    msg.payload.deviceId = deviceId;
                    broadcastToDashboards(msg.payload);
                    break;
                }
                ...
                case 'command_ack': {
                    ...
                    if (msg.files) {
                        try {
                            const filesArr = JSON.parse(msg.files);
                ...
                case 'webrtc_ice': {
                    if (isDashboard) {
                        // Dashboard sending to device
                        const targetDevice = msg.device_id;
                        if (targetDevice) sendToDevice(targetDevice, msg);
                    } else if (deviceId) {
                        // Device sending to dashboard
                ...
                default: {
                    // If dashboard sends a direct command to a device via WS
                    if (isDashboard && msg.target_device) {
                        const target = msg.target_device;
                        // Transform the Reference UI format {command: 'START_CAMERA'} to our format {type: 'command', action: 'camera_start'}
                        let action = msg.command;
                        if (action === 'START_CAMERA') action = 'camera_start';
                        if (action === 'STOP_CAMERA') action = 'camera_stop';
                        if (action === 'START_MIC') action = 'mic_start';
                        if (action === 'STOP_MIC') action = 'mic_stop';
                        if (action === 'START_RECORDING_CAMERA') action = 'record_video';
                        if (action === 'START_RECORDING_MIC') action = 'record_audio';
                        if (action === 'LOCK_DEVICE') action = 'lock';
                        if (action === 'GET_SNAPSHOT') action = 'refresh';
                        if (action === 'LIST_FILES') action = 'list_dir';
                        if (action === 'DOWNLOAD_FILE') action = 'download_file';
                        
                        // We must send the format the Android app expects
                        sendToDevice(target, {
                            type: 'command',
                            id: uuidv4(),
                            action: action,
                        ...
                        console.log(`[WS] Forwarded command ${action} to ${target}`);
                    }
                    break;
                }
            }
