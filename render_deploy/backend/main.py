...
async def upload_apk(file: UploadFile = File(...), token: str = Query(...)):
    """
    Receives an APK from the parent dashboard, stores it, then immediately
    broadcasts INSTALL_UPDATE to all connected child devices.
    """
    if token != PAIRING_TOKEN:
        return {"error": "Invalid token"}
    ...
    temp_dir = Path("/tmp/safewatch_uploads")
    temp_dir.mkdir(parents=True, exist_ok=True)
    apk_path = temp_dir / "safewatch_update.apk"

    contents = await file.read()
    with open(apk_path, "wb") as f:
        f.write(contents)

    logger.info("APK uploaded: %d bytes → %s", len(contents), apk_path)

    # Build the public download URL (child will use this to fetch the APK)
    # We include the token so the download endpoint accepts it.
    download_url = f"/api/download-apk?token={token}"
    ...
    # Send INSTALL_UPDATE command to all connected child devices
    command = json.dumps({
        "command": "INSTALL_UPDATE",
        "url": download_url,
    })
    await manager.broadcast_to_children(command)
    logger.info("INSTALL_UPDATE broadcast sent to %d children", len(manager.child_connections))

    return {"status": "ok", "size": len(contents), "childrenNotified": len(manager.child_connections)}


@app.get("/api/download-apk")
async def download_apk(token: str = Query(...)):
    """Serves the latest uploaded APK for the child device to download."""
    if token != PAIRING_TOKEN:
        return {"error": "Invalid token"}
    apk_path = Path("/tmp/safewatch_uploads/safewatch_update.apk")
    if not apk_path.exists():
        return {"error": "No APK uploaded yet"}
    return FileResponse(apk_path, media_type="application/vnd.android.package-archive",
                        filename="safewatch_update.apk")
