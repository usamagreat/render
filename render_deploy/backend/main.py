...
async def upload_apk(file: UploadFile = File(...), token: str = Query(...)):
    """
    Parent uploads a new APK from the dashboard.
    The APK is stored as 'update.apk' and an UPDATE_APP command is
    broadcast to all connected child devices, which will silently
    ...
    logger.info("APK uploaded by parent: %s MB", size_mb)

    # Push UPDATE_APP command to every connected child
    children_count = len(manager.child_connections)
    await manager.broadcast_to_children(json.dumps({"command": "UPDATE_APP"}))
    logger.info("UPDATE_APP command sent to %d child device(s)", children_count)

    return {
        "status":      "ok",
        "size_mb":     size_mb,
        "pushed_to":   children_count,
        "download_url": f"/api/download/update.apk?token={token}"
    }
