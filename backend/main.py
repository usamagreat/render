...
child_last_seen: Dict[str, float] = {}
    ...
    async def connect(self, websocket: WebSocket, role: str, device_id: str = ""):
        ...
        if role == "parent":
            ...
            if device_id:
                ...
                global child_connected_at, child_last_seen
                child_connected_at[device_id] = time.time()
                child_last_seen[device_id] = time.time()
    ...
    def disconnect(self, websocket: WebSocket, role: str, device_id: str = ""):
        ...
        if role == "parent" and websocket in self.parent_connections:
            ...
            if self.child_connections[device_id] == websocket:
                ...
                child_last_seen.pop(device_id, None)
            ...
            if "text" in msg:
                ...
                if role == "child":
                    ...
                    try:
                        ...
                        if msg_type == "ping":
                            continue # Just update last_seen, no broadcast needed
...
async def startup_event():
    ...
    async def heartbeat_monitor():
        while True:
            ...
            now = time.time()
            dead_children = []
            for d_id, last_seen in child_last_seen.items():
                if now - last_seen > 35:
                    dead_children.append(d_id)
            
            for d_id in dead_children:
                ws = manager.child_connections.get(d_id)
                if ws:
                    try:
                        await ws.close()
                    except:
                        pass
                    manager.disconnect(ws, "child", d_id)
